	...
	function get_templates($template, $type='')
	{
		...
		switch($type) {
			case 'more':
				...
				if(!$template_post) {
					$template_post = '<a href="{search_url_escaped}"><span class="sf_text">'.esc_html__('See more results','michigan').'</span><span class="sf_small">'.esc_html__('Displaying top results','michigan').'</span></a>';
				}
				break;
			case 'taxonomy':
			...
			case 'post_type':
				...
				if(!$template_post && in_array($template, self::$woocommerce_post_types)) {
					$template_post = '<a href="{post_link}">{post_image_html}<span class="sf_text">{post_title} - {price}</span><span class="sf_small">'.esc_html__('Posted by','michigan').' {post_author} '.esc_html__('on','michigan').' {post_date_formatted}</span></a>';
				}elseif(!$template_post){
					$template_post = '<a href="{post_link}">{post_image_html}<span class="sf_text">{post_title} </span><span class="sf_small">'.esc_html__('Posted by','michigan').' {post_author} '.esc_html__('on','michigan').' {post_date_formatted}</span></a>';
				}
				break;
			default:
				$template_post = get_option('sf_template_'.$template);
