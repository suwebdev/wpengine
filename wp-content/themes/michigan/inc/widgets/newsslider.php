<?php class WebnusPostSliderWidget extends WP_Widget{
	...
	public function form($instance){
		...
		<p><label for="<?php echo $this->get_field_id('count') ?>"><?php esc_html_e('Posts Count: ','michigan') ?></label><input type="text"	class="widefat"	id="<?php echo $this->get_field_id('count') ?>"	name="<?php echo $this->get_field_name('count') ?>"	value="<?php if( isset($count) )  echo esc_attr($count); ?>"/></p>
		<?php 
	}
		...
		if(!empty($title))
			...
			if($query->have_posts()){
				while($query->have_posts()) 
				{
					...
					$output .= get_the_image( array( 'meta_key' => array( 'thumbnail', 'thumbnail' ), 'size' => 'michigan_webnus_latest_img' , 'link_to_post' => false , 'echo'=>false) );
...
add_action('widgets_init','register_michigan_webnus_postslider_widget'); 
function register_michigan_webnus_postslider_widget(){
