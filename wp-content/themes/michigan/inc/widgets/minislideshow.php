<?php class WebnusMiniSlideShowWidget extends WP_Widget{
	...
	extract($instance); ?>
		...
		<p><label for="<?php echo $this->get_field_id($textboxes) ?>">Slide #<?php echo $i; ?>"><?php esc_html_e('Image URL: ','michigan') ?></label><input type="text" class="widefat" id="<?php echo $this->get_field_id($textboxes) ?>" name="<?php echo $this->get_field_name($textboxes) ?>" value="<?php if( isset($$$textboxes) )  echo esc_attr($$textboxes); ?>"/></p>
		<?php }
	}
...
add_action('widgets_init','register_michigan_webnus_minislideshow_widget'); 
function register_michigan_webnus_minislideshow_widget(){
