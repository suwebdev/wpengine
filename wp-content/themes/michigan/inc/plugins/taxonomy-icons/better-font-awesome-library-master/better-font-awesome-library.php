...
class Better_Font_Awesome_Library {
	...
	function setup_root_url() {
		...
		$bfa_directory = get_template_directory() . '/inc/plugins/taxonomy-icons/better-font-awesome-library-master';
	...
	private function fetch_api_data( $url ) {
		...
		if ( false === ( $response = get_transient( self::SLUG . '-api-versions' ) ) ) {
			...
			if ( 200 == wp_remote_retrieve_response_code( $response ) ) {
				...
				$this->set_error( 'api', 'Unknown', esc_html__( 'The jsDelivr API servers appear to be temporarily unavailable.', 'michigan' ) . " (URL: $url)" );
				$response = '';
	...
	public function add_insert_shortcode_button() {
		...
		<span class="bfa-iconpicker fontawesome-iconpicker" data-selected="fa-flag">
			<a href="#" class="button button-secondary iconpicker-component">
				...
				<?php esc_html_e( 'Insert Icon', 'michigan' ); ?>
	...
	public function do_admin_notice() {
		...
		if ( ! empty( $this->errors ) && apply_filters( 'bfa_show_errors', true ) ) :
		    ...
		    <div class="error">
		    	<p>
		    		<b><?php esc_html_e( 'Better Font Awesome', 'michigan' ); ?></b>
		    	</p>

	        	<!-- API Error -->
	        	<?php if ( is_wp_error ( $this->get_error('api') ) ) : ?>
		        	<p>
		        		<b><?php esc_html_e( 'API Error', 'michigan' ); ?></b><br />
		        		<?php
		        		printf( esc_html__( 'The attempt to reach the jsDelivr API server failed with the following error: %s', 'michigan' ),
	        	...
	        	<?php if ( is_wp_error ( $this->get_error('css') ) ) : ?>
		        	<p>
		        		<b><?php esc_html_e( 'Remote CSS Error', 'michigan' ); ?></b><br />
		        		<?php
		        		printf( esc_html__( 'The attempt to fetch the remote Font Awesome stylesheet failed with the following error: %s %s The embedded fallback Font Awesome will be used instead (version: %s).', 'michigan' ),
		        ...
		        <!-- Fallback Text -->
		        <p><?php echo __( '<b>Don\'t worry! Better Font Awesome will still render using the included fallback version:</b> ', 'michigan' ) . '<code>' . $this->fallback_data['version'] . '</code>' ; ?></p>

		        <!-- Solution Text -->
		        <p>
		        	<b><?php esc_html_e( 'Solution', 'michigan' ); ?></b><br />
			        <?php
			        printf( esc_html__( 'This may be the result of a temporary server or connectivity issue which will resolve shortly. However if the problem persists please file a support ticket on the %splugin forum%s, citing the errors listed above. ', 'michigan' ),
