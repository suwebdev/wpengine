...
while ( $my_query->have_posts() ) {
    ...
    if ( $link != 'link_no' ) {
        if ( $link == 'link_post' ) {
            $link_image_start = '<a class="link_image" href="'.get_permalink($post_id).'" title="'.sprintf( esc_attr__( 'Permalink to %s', 'michigan' ), the_title_attribute( 'echo=0' ) ).'">';
        }
...
else { $teasers = esc_html__('Nothing found.' , 'michigan'); }
